...
from data.scenario_bundle import build_observation_bundle_from_precomputed, load_or_build_scenario_bundle
...
from orbit.rtn import build_eci_to_rtn_matrices, project_batch_rtn_to_eci
...
def main() -> None:
    ...
    bundle = load_or_build_scenario_bundle(config=config, cache_dir=output_dirs["cache"], logger=logger)
    bundle_metadata = bundle["metadata"]
    scenario_times_sec = np.asarray(bundle["scenario_times_sec"])
    train_mask = np.asarray(bundle["train_mask"]).astype(bool)
    ...
    if config.precomputed_bundle_path:
        logger.info(
            ...
            bundle_metadata["satellite_name"],
            bundle_metadata["norad_id"],
            bundle_metadata["tle_epoch_utc"],
            bundle_metadata["scenario_start_utc"],
        )
        logger.info("Orekit data source: %s", bundle_metadata["orekit_data_path"])

    r_sgp4_eci_m = np.asarray(bundle["r_sgp4_eci_m"])
    v_sgp4_eci_mps = np.asarray(bundle["v_sgp4_eci_mps"])
    r_truth_eci_m = np.asarray(bundle["r_truth_eci_m"])
    v_truth_eci_mps = np.asarray(bundle["v_truth_eci_mps"])
    clean_delta_r_rtn_m = np.asarray(bundle["clean_delta_r_rtn_m"])
    clean_delta_v_rtn_mps = np.asarray(bundle["clean_delta_v_rtn_mps"])
    ...
    observation_bundle = build_observation_bundle_from_precomputed(bundle=bundle, predict_velocity=config.predict_velocity)
    ...
    metrics = {
        "metadata": {
            "satellite_name": bundle_metadata["satellite_name"],
            "norad_id": bundle_metadata["norad_id"],
            "tle_epoch_utc": bundle_metadata["tle_epoch_utc"],
            "scenario_start_utc": bundle_metadata["scenario_start_utc"],
            "common_inertial_frame": bundle_metadata["common_inertial_frame"],
            ...
            "orekit_data_path": bundle_metadata["orekit_data_path"],
            "synthetic_truth_note": bundle_metadata["synthetic_truth_note"],
            "truth_force_models": bundle_metadata["truth_force_models"],
        },
        "train_segment": {
